	...
	$("body").on('click',".del-imag",function(e){
		e.preventDefault();
		console.log("borrando su origen");
		/*var id = $(this).data("id");
				$.ajax({
				    url: 'del-img',
				    data: {id:id},
	...
	$('body').on('change', '#file', function() {
		...
		if(extensiones.toLowerCase() != ".jpg" && extensiones.toLowerCase() != ".jpeg" && extensiones.toLowerCase() != ".png" && extensiones.toLowerCase() != ".mp4")
		{
			...
			swal(
			  '',
			  'Solo puedes subir formatos .jpg, .png',
			  'error'
			)
		}else{
			if(extensiones.toLowerCase() == ".jpg" || extensiones.toLowerCase() == ".jpeg" || extensiones.toLowerCase() == ".png" ){
				var fil = this.files[0];
				if(fil.size < 6500000 ){
					if (this.files && this.files[0]) {
						...
						//add input hide with base64
					      reader.onload = function (e) {
					      	...
					      	$('#previewimg' + abc).attr('src', target);
					      	/*
					      	 *
					      	 *  TEST
					      	 *
					...
					function progress(e){
						...
						if(e.lengthComputable){
							...
							if(Percentage >= 100)
							{
								$("#progress").attr('data-original-title', percentReal);
								$('.tool').tooltip({placement: 'bottom',trigger: 'manual'}).tooltip('show');
								$('#progress-message').text('Se completo el proceso,espera un momento..');
								...
								console.log("Go to upload img");
								//Upload imagen base64
								//console.log(target);
									$.ajax({
									    url: 'cls',
									    data: { imagen:target, name:name, dir : dir},
									    type: 'POST',
										xhr: function() {
									    ...
									    success: function(msg) {
											...
											var ruta_complet = "public/projects/"+msg.dir+"/"+name;
											console.log(ruta_complet);
											$("#abcd" + abc).append( "<img src='assets/images/close.png' data-id='"+ruta_complet+"' data-toggle='modal' data-target='#del-imag' class='close-del' width='40'>");
											$("#progress-message").text("Imagen guardada exitosamente.");
				...
				//reader.onload = imageIsLoaded;
						...
						reader.readAsDataURL(this.files[0]);
						 $(this).hide();
						// $("#abcd" + abc).append( "<img src='assets/images/close.png' data-id='"+msg.dir+"' data-toggle='modal' data-target='#del-imag' width='40'>");
						//$("#abcd" + abc).data('toogle','modal');
